    ...
    <div class="articles">
      <div class="article" v-for="page in files">
        ...
        <div class="description">
          <span class="action" @click="toggle(page)" v-if="!page.show">Click to see details</span>
          <div v-if="page.show">
            {{page.frontmatter.description.content}}
          </div>
          <div class="action" @click="toggle(page)" v-if="page.show">Hide details</div>
        </div>
    ...
    files() {
      ...
      sortedPages
        ...
        if(this.sortKey === 'newest') {
          sortedPages = sortedPages.slice(0,5);
        } else if(this.sortKey) {
          sortedPages = sortedPages
            .filter(p => p.frontmatter[this.sortKey])
        }
        sortedPages = sortedPages.map(p => ({ ...p, show:false}))
